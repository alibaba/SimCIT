...
from modules.kmeans import kmeans_init_
...
class VQBottleneck(nn.Module):
    ...
    def forward(self, x, temperature):
        ...
        if self.training:
            ...
            commit_loss = ((x - emb.detach()) ** 2).sum(axis=[-1])
        
        else:
            emb_out = self.get_item_embeddings(ids)
            # loss = self.quantize_loss(query=x, value=emb_out)
            emb_loss = ((x.detach() - emb_out) ** 2).sum(axis=[-1])
            commit_loss = ((x - emb_out.detach()) ** 2).sum(axis=[-1])

        return QuantizeOutput(emb_out,
                              ids,
                              emb_loss,
                              commit_loss)
